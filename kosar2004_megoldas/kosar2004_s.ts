...
interface Adat {
    ...
    idegen_pont: number;
    helyszin: string;
    merkozes_idopontja: string;
}

function ObjektumFeltolto(feltoltendoElem: string[]): Adat[] {
    let beolvasottAdatok: Adat[] = [];
    for (let i: number = 0; i < feltoltendoElem.length; i++) {
        let daraboltAdatok: string[] = feltoltendoElem[i].split(";");
        let uj_eredmenyek: Adat = {
            ...
            merkozes_idopontja: daraboltAdatok[5],
        }
        beolvasottAdatok.push(uj_eredmenyek);
    }
    return beolvasottAdatok;
}
const Eredmenyek: Adat[] = ObjektumFeltolto(eredmenyek);

//3. feladat

function Real_kereso(vizsgaltTomb: Adat[]): [number, number] {
    let hazai_real: number = 0;
    let idegen_real: number = 0;
    ...
    return [hazai_real, idegen_real];
}

function real_kiirato(Real: [number, number]): void {
    var Real = Real_kereso(Eredmenyek);
    console.log("3. feladat: Real Madrid: Hazai: " + Real[0] + ", Idegen: " + Real[1]);
}
real_kiirato(Real_kereso(Eredmenyek));

//4. feladat

function dontetlen_kereso(vizsgaltTomb: Adat[]): boolean {
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
...
function dontetlen_kiirato(eredmeny: boolean): void {
    if (eredmeny == true) {
...
dontetlen_kiirato(dontetlen_kereso(Eredmenyek));

//5. feladat

function barcelona_kereso(vizsgaltTomb: Adat[]): string {
    let keresett_csapat: string = "";
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        if (vizsgaltTomb[i].hazai.includes("Barcelona")) {
            keresett_csapat = vizsgaltTomb[i].hazai;
        }
    }
    return keresett_csapat;
}

function barcelona_kiirato(eredmeny: string): void {
    console.log("5. feladat: barcelonai csapat neve: " + eredmeny)
}

barcelona_kiirato(barcelona_kereso(Eredmenyek));
...
function november2004(vizsgaltObjektum: Adat[]): November2004[] {
    let csapatInfo: November2004[] = [];
    for (let i: number = 0; i < vizsgaltObjektum.length; i++) {
        if (vizsgaltObjektum[i].merkozes_idopontja == "2004-11-21") {
            let ujCsapat: November2004 = {
                hazaiCsapat: vizsgaltObjektum[i].hazai,
                idegenCsapat: vizsgaltObjektum[i].idegen,
                hazaiPont: vizsgaltObjektum[i].hazai_pont,
                idegenPont: vizsgaltObjektum[i].idegen_pont,
            }
...
function November2004Kiirato(eredmeny: November2004[]): void {
    console.log("6. feladat: ")
    for (let i: number = 0; i < eredmeny.length; i++) {
        console.log(eredmeny[i].hazaiCsapat + "-" + eredmeny[i].idegenCsapat + " " + eredmeny[i].hazaiPont + "-" + eredmeny[i].idegenPont)
    }
}
November2004Kiirato(november2004(Eredmenyek));

//7. feladat

function stadionok(vizsgaltObjektum: Adat[]): string[] {
    let stadionok: string[] = [];
    for (let i: number = 0; i < vizsgaltObjektum.length; i++) {
        let szerepelE: boolean = false;
        for (let j: number = 0; j < stadionok.length; j++) {
            if (vizsgaltObjektum[i].helyszin == stadionok[j]) {
                szerepelE = true;
            }
        }
        if (szerepelE == false) {
            stadionok.push(vizsgaltObjektum[i].helyszin)
        }
    }
    return stadionok;
}

function merkozesekMennyisege(vizsgaltObjektum: Adat[], stadionok: string[]): number[] {
    let merkozesekMennyisege: number[] = [];
    for (let i: number = 0; i < stadionok.length; i++) {
        merkozesekMennyisege.push(0);
    }

    for (let i: number = 0; i < vizsgaltObjektum.length; i++) {
        for (let j: number = 0; j < stadionok.length; j++) {
            if (vizsgaltObjektum[i].helyszin == stadionok[j]) {
                merkozesekMennyisege[j]++;
            }
        }
    }
    return merkozesekMennyisege;
}

function stadionStatisztikaKiir(eredmenyTomb: string[], mennyisegTomb: number[]): void {
    console.log("7. feladat");
    for (let i: number = 0; i < mennyisegTomb.length; i++) {
        if (mennyisegTomb[i] > 20) {
            console.log(eredmenyTomb[i] + ": " + mennyisegTomb[i]);
        }

    }
}

stadionStatisztikaKiir(stadionok(Eredmenyek), merkozesekMennyisege(Eredmenyek, stadionok(Eredmenyek)));